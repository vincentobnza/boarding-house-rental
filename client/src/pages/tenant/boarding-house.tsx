...
const RentalHouseCard = ({ house }: RentalHouseCardProps) => {
  return (
    <Link to={`/tenant/rental-house/${house.id}`} className="block">
      <div className="group relative h-90 w-full cursor-pointer overflow-hidden rounded-lg bg-black shadow-2xl transition-transform duration-300 hover:scale-[1.02]">
        <img
          src={house.image}
          alt="image"
